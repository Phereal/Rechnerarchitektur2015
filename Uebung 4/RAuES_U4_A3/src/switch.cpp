...
void Switch::distribute()
{
  ...
  for(int i = 0; i < n; ++i)
  {
    ...
    if( pPending[i].read())
    {
      // try to save package in buffer first (buffer not full)
      ...
      if(in[i].read().addr < m)
      {
        ...
        if(!cBusy[in[i].read().addr].read())
        {
          ...
          if(bufferCount[in[i].read().addr] > 0)
          {
            ...
            out[ cBusy[in[i].read().addr] ] = ReadPackageFromBuffer(cBusy[in[i].read().addr]);
            //pReceived[ cBusy[in[i].read().addr] ] = true;
          }
        }
        else
        {
          // consumer busy, do nothing
        }
      }
      else
      {
        // address out of range, do ...
        // todo
      }
    }
  }
